		...
		const unsigned colortable[768])
{
	...
	GLfloat *vtx = malloc(3*num_particles*3*sizeof(GLfloat));
	GLfloat *tex = malloc(2*num_particles*3*sizeof(GLfloat));
	GLfloat *clr = malloc(4*num_particles*3*sizeof(GLfloat));
...
R_DrawParticles(void)
{
	...
	if (gl_config.pointparameters && !(stereo_split_tb || stereo_split_lr))
	{
		...
		GLfloat *vtx = malloc(3*r_newrefdef.num_particles*sizeof(GLfloat));
		GLfloat *clr = malloc(4*r_newrefdef.num_particles*sizeof(GLfloat));
		unsigned int index_vtx = 0;
		unsigned int index_clr = 0;
